...
class RegularStarPolygonCreator {
    ...
    static Path createRegularStarPolygon(int left, int top, int right, int bottom,
        ...
        if (outline) {
            ...
            float innerRadius = LineMath.distance(radius, radius, firstIntersection[0],
                    firstIntersection[1]);
    ...
    private static float[] findFirstIntersectionPoint(float[][] pointsArray) {
        ...
        float firstSlope = LineMath.slope(firstPt1, firstPt2);
        float firstYInt = LineMath.yIntercept(firstPt1, firstSlope);
        ...
        for (int i = 2; i < pointsArray.length - 1; i++) {
            ...
            float curSlope = LineMath.slope(curPt1, curPt2);
